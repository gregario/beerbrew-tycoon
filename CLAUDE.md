...
- **Stage 6 (Meta-Progression) complete** — MetaProgressionManager autoload (unlock points, 4 unlock categories: styles/blueprints/ingredients/perks, 6 achievements, run history, meta.json persistence). RunModifierManager autoload (perk/modifier effects). 5 new UI overlays: RunSummaryOverlay, UnlockShopOverlay, RunStartOverlay, AchievementsOverlay, MainMenu. Game flow: GameOver → RunSummary → UnlockShop → RunStart → reset. Blueprint research discounts, meta style unlocks. 701/701 GUT tests.
- **Integration & polish complete** — Full run integration tests, meta-persistence tests, balance reference document (`docs/balance-reference.md`), UI polish pass (all Stage 6 overlays normalized to design system). All stages 1-6 implemented.
- **Brewing Depth Expansion complete** — 7-component quality scoring (Style 25%, Fermentation 25%, Science 15%, Water 10%, Hops 10%, Novelty 10%, Conditioning 5%), 16 beer styles (9 new), water chemistry system (5 profiles), hop scheduling (4 slots), conditioning system (0-4 weeks with off-flavor decay), off-flavor spectrum (float 0-1 intensities, 6 types with per-week decay), progressive revelation via measurement equipment (5 items), yeast-temp interaction and tolerance scoring, discovery extensions (water/hop/yeast-flavor), 31-node research tree (16 techniques, 4 ingredients, 4 equipment, 7 styles). Economy simulation tests verify win achievable within 60 turns. 961/961 GUT tests.
- OpenSpec change `post-mvp-roadmap` — all 6 stages implemented.
...
- Manual tasks outstanding: 14.6 (60fps profiler check), 14.7 (pixel aliasing at 1080p), 6.1–6.5 (UI folder restructure via Godot editor), manual playtests (full run + meta-persistence across 2 runs).
